...
class GradMod_2Modality(nn.Module):
    def __init__(self, cfgs):
        super().__init__()
        self.extract_mm_feature = False
        self.mode = 'train'
        # Keeps the same base architecture but we will ignore Audio
        if cfgs.fusion_type == "late_fusion":
            self.net = Later_Fusion_Model(cfgs)
        elif cfgs.fusion_type == "early_fusion":
            self.net = Early_Fusion_Model(cfgs)
        ...
        # Register hooks only for Vision and Text
        self.m_v_o.register_full_backward_hook(self.hookv)
        self.m_t_o.register_full_backward_hook(self.hookt)

    def hookv(self, m, ginp, gout):
        return ginp[0].clone() * self.scale_v,
    
    def hookt(self, m, ginp, gout):
        return ginp[0].clone() * self.scale_t,

    def update_scale(self, coeff_v, coeff_t):
        self.scale_v = coeff_v
        self.scale_t = coeff_t

    def forward(self, vision, audio, text, feature_length):
        # Force Audio to zero to ensure it contributes nothing to the fusion
        audio = torch.zeros_like(audio) 
        
        self.net.mode = "feature"
        total_out, encoded_feature = self.net(vision, text, feature_length, pad_visual=False, pad_text=False)
        
        self.net.mode = "classify"
        self.net.eval()
        
        # 2-Modality specific padding passes
        # pad_visual_out means Vision is zeroed, only Text remains
        pad_visual_out = self.net(vision, text, feature_length, pad_visual=True, pad_text=False)
        # pad_text_out means Text is zeroed, only Vision remains
        pad_text_out = self.net(vision, text, feature_length, pad_visual=False, pad_text=True)
        # zero_padding means both Vision and Text are zeroed
        zero_padding_out = self.net(vision, text, feature_length, pad_visual=True, pad_text=True)
        
        if self.mode == 'train':
            self.net.train()

        # AGM 2-Modality Formula (Reference: CREMA-D implementation)
        # m_v = 0.5 * (Total - Missing_V + Missing_T)
        m_v_out = self.m_v_o(0.5 * (total_out - pad_visual_out + pad_text_out))
        m_t_out = self.m_t_o(0.5 * (total_out - pad_text_out + pad_visual_out))

        if self.extract_mm_feature:
            return None, None, None, None, m_v_out, m_t_out, m_v_out + m_t_out, encoded_feature
        else:
            return None, None, None, None, m_v_out, m_t_out, m_v_out + m_t_out
...
class Later_Fusion_Model(nn.Module):
    ...
    def __init__(self,cfgs):
        ...
        # FIX: Changed 2304 to 1536 (768 for Vision + 768 for Text)
        self.head = MLP(1536, 1024, 2, one_layer=True)
        
        
    def forward(self,vision,text,feature_length,pad_visual=False,pad_text=False):
        # if pad_audio:
        ...
        encoded_out = torch.cat([vision_encoded_out,text_encoded_out],dim=1)
